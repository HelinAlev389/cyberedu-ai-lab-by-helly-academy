...
from flask import (
    ...
    redirect, url_for, current_app, g, session
)
...
def load_ai_session():
    if not request.path.startswith('/ai_teacher'):
        return

    if not current_user.is_authenticated:
        return

    # ❗ Само студенти имат достъп до AI-учителя
    if current_user.role != 'student':
        flash("Само студентите имат достъп до AI уроците.", "danger")
        return redirect(url_for('teacher_dashboard.dashboard'))  # или друга подходяща дестинация

    # Създаване на AI сесия, ако липсва
...
@login_required
def dashboard():
    last_step = getattr(g, "ai_session", None)
    return render_template('ai_teacher.html', last_step=last_step.step if last_step else "lesson")
...
def message():
    ...
    try:
        ...
        full_prompt = (
            ...
            f"{context_block}\n\n"
            f"Нов въпрос от потребителя:\n{user_text}"
        )

        reply = ai_service.chat(full_prompt)
...
def lesson():
    ...
    try:
        lesson_content = ai_service.chat(prompt_lesson)
        quiz_json = ai_service.chat(prompt_quiz)
        quiz_questions = json.loads(quiz_json)
    except Exception as e:
        flash(f"Грешка при зареждане на урок или тест: {e}", 'danger')
...
def exercise():
    ...
    prompt_grade = (
        "Оцени и обясни всеки студентски отговор (0–100%) и дай кратка обратна връзка. "
        "Използвай JSON: {\"results\": [{\"id\": \"q1\", \"score\": 85, \"feedback\": \"Добър, но непълен отговор\"}, ...], \"average\": X}.\n"
        + answer_lines
    )

    try:
        grade_resp = ai_service.chat(prompt_grade)
    ...
    checkpoint_score = g.ai_session.config.get("checkpoint", 50)

    if avg_score < checkpoint_score:
        flash(
            f"Резултат {avg_score:.1f}% (праг {checkpoint_score}%). "
    ...
    try:
        exercises = ai_service.chat(prompt_ex)
    except Exception as e:
        flash(f"Грешка при генериране на упражнения: {e}", "danger")
        return redirect(url_for('ai_teacher.lesson'))

    return render_template('ai_exercises.html',
                           results=results,
                           average=avg_score,
                           exercises=exercises)
